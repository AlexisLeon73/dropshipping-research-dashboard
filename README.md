...
**Only Google Trends is implemented.** Reddit, Meta Ads Library, TikTok,
...
| Reddit | 🚧 Stub only | Official OAuth API, needs `REDDIT_CLIENT_ID` / `REDDIT_CLIENT_SECRET`. |
...
etc.). That means the actual live call to `trends.google.com` could not be
exercised here — a real run there returns a `ProxyError`/403, which is
this sandbox's network policy, not a bug in the code. Everything else was
tested end-to-end for real: the live FastAPI server was started and hit
with real HTTP requests (index → search → results → CSV export → history),
and a mocked Google Trends source (verified to run through the exact same
scoring/storage code as the real one) exercises the full pipeline in
`tests/test_pipeline.py`. When you run this locally with normal internet
access, `pytrends` will make real calls to Google Trends — you may still
see occasional 429s (see above), which the retry/cache logic is built to
absorb.

## Requirements

- Python 3.11+
- Internet access (for the Google Trends calls)
...
cp .env.example .env   # optional — no env vars are required for Google Trends
```
...
app/
  ...
  sources/
    base.py             SignalSource interface every source implements
    google_trends.py     Implemented
    reddit.py             Stub — integration plan in the docstring
...
## Adding the next source (Reddit or Meta Ads)
...
3. Add the source instance to `ACTIVE_SOURCES` in `app/main.py`.

Nothing else needs to change: `run_search()` already loops over every
configured, implemented source, merges signals per term, and combines
scores via `scoring.combine_source_scores()`.
